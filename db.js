...
let _sqliteDb = null;
let _pgPool = null;
const _usePg = !!process.env.DATABASE_URL;
...
function sqliteSql(sql) {
  // Convert NOW() back to SQLite datetime for SQLite mode
...
const db = {
  async all(sql, params = []) {
    if (_usePg) {
      const result = await _pgPool.query(pgSql(sql), params);
      return result.rows;
    } else {
      return _sqliteDb.prepare(sqliteSql(sql)).all(...params);
    }
  },
  ...
  async get(sql, params = []) {
    if (_usePg) {
      ...
      return _sqliteDb.prepare(sqliteSql(sql)).get(...params);
    }
  },
  ...
  async run(sql, params = []) {
    if (_usePg) {
      ...
      const result = await _pgPool.query(pgQuery, params);
      if (isInsert && result.rows && result.rows[0]) {
        return { lastInsertRowid: result.rows[0].id };
      }
      return { changes: result.rowCount };
    } else {
      const result = _sqliteDb.prepare(sqliteSql(sql)).run(...params);
...
async function init() {
  if (_usePg) {
    ...
    const safeAlter = (sql) => { try { _sqliteDb.exec(sql); } catch(e) { /* column likely exists */ } };
...
async function seedIfEmpty() {
  // Seed vendors
  const vendorRow = await db.get('SELECT COUNT(*) as c FROM vendors');
  const vendorCount = parseInt(vendorRow.c, 10);
  if (vendorCount === 0) {
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Shaw Industries', 'Mike Davis', '800-441-7429', 'mdavis@shaw.com', 'tile/carpet/LVT']);
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Dal-Tile Corporation', 'Lisa Chen', '800-933-8453', 'lchen@daltile.com', 'tile']);
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Mohawk Group', 'Tom Bradley', '800-622-6227', 'tbradley@mohawkgroup.com', 'carpet/LVT']);
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Custom Building Products', 'Jim Harris', '800-272-8786', 'jharris@cbpmail.net', 'setting materials']);
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Bostik', 'Anna White', '800-726-7845', 'awhite@bostik.com', 'adhesives']);
    await db.run(`INSERT INTO vendors (name, rep_name, phone, email, notes) VALUES (?, ?, ?, ?, ?)`,
      ['Schluter Systems', 'Karl Mueller', '800-472-4588', 'kmueller@schluter.com', 'transitions/trim']);
  }

  // Seed assembly catalog
  const countRow = await db.get('SELECT COUNT(*) as c FROM assembly_catalog');
  const count = parseInt(countRow.c, 10);
  if (count === 0) {
    const vendors = {};
    const vRows = await db.all('SELECT id, name FROM vendors');
    vRows.forEach(v => { vendors[v.name] = v.id; });
  ...
  const pcRow = await db.get('SELECT COUNT(*) as c FROM product_catalog');
  const pcCount = parseInt(pcRow.c, 10);
  if (pcCount === 0) {
    const vendors = {};
    const vRows = await db.all('SELECT id, name FROM vendors');
    vRows.forEach(v => { vendors[v.name] = v.id; });
    ...
    const pcItems = [
      ['24x24 Porcelain Floor Tile', 'tile', 'SF', shawId, 4.20, 30, 7, 0, 0, 20],
      ['12x24 Ceramic Wall Tile', 'tile', 'SF', dalTileId, 3.80, 30, 7, 0, 0, 20],
      ['LVT Plank 6x48', 'LVT', 'SF', shawId, 3.50, 30, 7, 0, 0, 20],
      ['Broadloom Carpet', 'carpet', 'SY', mohawkId, 2.80, 30, 7, 0, 0, 20],
      ['Carpet Tile 24x24', 'carpet', 'SF', mohawkId, 3.20, 30, 7, 0, 0, 20],
      ['Rubber Base 4in', 'base', 'LF', schluterID, 2.10, 30, 7, 0, 0, 20],
      ['Marble Threshold', 'transitions', 'each', dalTileId, 18, 30, 7, 0, 0, 20],
      ['T-Mold Transition', 'transitions', 'LF', schluterID, 12, 30, 7, 0, 0, 20],
    ];

    for (const row of pcItems) {
      await db.run(
        `INSERT INTO product_catalog (name, category, unit, vendor_id, default_material_cost, default_material_markup, default_tax_rate, default_vendor_fee, default_labor_cost, default_labor_markup) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        row
      );
    }
  }
  ...
  // Seed trade partners
  const tpRow = await db.get('SELECT COUNT(*) as c FROM trade_partners');
  const tpCount = parseInt(tpRow.c, 10);
  if (tpCount === 0) {
    await db.run(`INSERT INTO trade_partners (name, contact_name, trade) VALUES (?, ?, ?)`, ['Tile Masters LLC', 'Mike Torres', 'tile']);
    await db.run(`INSERT INTO trade_partners (name, contact_name, trade) VALUES (?, ?, ?)`, ['Premier Carpet Installers', 'Sarah Kim', 'carpet']);
    await db.run(`INSERT INTO trade_partners (name, contact_name, trade) VALUES (?, ?, ?)`, ['Pro Flooring Solutions', 'Dave Wilson', 'LVT']);
  }
}
...
module.exports = { init, seedIfEmpty, getDb, getPool, getDefaultWoTerms, db };
